...
data_models=(cifar10_res0.01im cifar100_res0.01im)
gpu=3
...
if [ "$1" = "MSP" ]; then
	...
	model=${array[1]}
        for seed in 1; do
            #for quantile in $(seq $initial $inc $end); do
		...
		n=4	
	            ...
	            echo "---------------"$m_in"------"$m_out"--------------------"
	            CUDA_VISIBLE_DEVICES=$gpu python BE_train_im_resD.py $data --model $model --score $score --seed $seed --m_in $m_in --m_out $m_out --gamma1 $gamma1 --gamma2 $gamma2 --lamb $lamb --trial $trial --beta1 $beta1 --beta2 $beta2 --loss_mode $loss_mode --dstemp $dstemp --quantile $quantile --n $n --step_size $step_size
	            CUDA_VISIBLE_DEVICES=$gpu python E_test_SC_resD.py --method_name ${dm}_s${seed}_$1 --num_to_avg 1 --score $score --trial $trial --gamma1 $gamma1 --gamma2 $gamma2  --beta1 $beta1 --beta2 $beta2 --loss_mode $loss_mode --dstemp $dstemp --quantile $quantile --n $n
	      #done     
	   done         
        done
    done
