...
if [ "$1" = "MSP" ]; then
	...
	model=${array[1]}
		...
		gamma2=0.75
	            ...
	            echo "---------------"$m_in"------"$m_out"--------------------"
	            CUDA_VISIBLE_DEVICES=$gpu python BE_train_im_resc.py $data --model $model --score $score --seed $seed --m_in $m_in --m_out $m_out --gamma1 $gamma1 --gamma2 $gamma2 --lamb $lamb --trial $trial
	            CUDA_VISIBLE_DEVICES=$gpu python E_test_SC_resc.py --method_name ${dm}_s${seed}_$1 --num_to_avg 1 --score $score --trial $trial --gamma1 $gamma1 --gamma2 $gamma2 
           done
        done
    done
