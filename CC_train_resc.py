...
# Checkpoints
parser.add_argument('--save', '-s', type=str, default='./snapshots/O_base_con/', help='Folder to save checkpoints.')
...
parser.add_argument('--num_ood_samples', default=30000, type=int, help='Number of OOD samples to use.')
...
def train():
    ...
    for in_set, out_set in zip(train_loader_in, train_loader_out):
        ...
        if args.dataset == 'cifar10':
            A_tr = 0.9369  # Training accuracy of CIFAR-10 baseline model
        else:
            A_tr = 0.7569  # Training accuracy of CIFAR-100 baseline model
...
for epoch in range(0, args.epochs):
    ...
    torch.save(net.state_dict(),
               os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model + '_s' + str(args.seed) +
                            '_' + save_info + '_epoch_' + str(epoch) + '_CC_trial_' + str(
                   args.trial) + '.pt'))

    # Let us not waste space and delete the previous model
    prev_path = os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model + '_s' + str(args.seed) +
                             '_' + save_info + '_epoch_' + str(epoch - 1) + '_CC_trial_' + str(args.trial) + '.pt')
