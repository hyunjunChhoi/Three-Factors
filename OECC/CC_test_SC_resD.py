...
parser.add_argument('--load', '-l', type=str, default='./snapshots/D_all/', help='Checkpoint path to resume / test.')
...
parser.add_argument('--beta1', type=float, default=1.0, help='contrastive loss factor')
parser.add_argument('--beta2', type=float, default=1.0, help='contrastive reg loss factor')
...
if args.load != '':
    for i in range(1000 - 1, -1, -1):
        if 'pretrained' in args.method_name:
            ...
            subdir = 'oecc_tune'
            new_method_name=args.method_name.replace('oe_tune', 'oecc_tune')
        elif 'energy_ft' in args.method_name:
            subdir = 'energy_ft'
        else:
            subdir = 'oe_scratch'
        
        model_name = os.path.join(os.path.join(args.load, subdir), new_method_name + '_epoch_' + str(i)+'_dstemp_'+str(args.dstemp)+ '_quantile_' + str(args.quantile)+'_loss_mode_'+str(args.loss_mode)+'_times_'+str(args.n)+ '_beta_' + str(args.beta1)+ str(args.beta2)+'_CC_trial_'+str(args.trial)  + '.pt')
...
# /////////////// OOD Detection ///////////////
auroc_list_SCOOD, aupr_list_SCOOD, fpr_list_SCOOD = [], [], []
auroc_list, aupr_list, fpr_list = [], [], []
auroc_list_syn, aupr_list_syn, fpr_list_syn = [], [], []
...
def get_and_print_results_syn(logger, ood_loader, num_to_avg=args.num_to_avg):
    ...
    print(in_score[:3], out_score[:3])
    auroc = np.mean(aurocs); aupr = np.mean(auprs); fpr = np.mean(fprs)
    auroc_list_syn.append(auroc); aupr_list_syn.append(aupr); fpr_list_syn.append(fpr)
...
def get_and_print_results_SCOOD(logger, ood_loader, num_to_avg=args.num_to_avg):
    ...
    print(in_score[:3], out_score[:3])
    auroc = np.mean(aurocs); aupr = np.mean(auprs); fpr = np.mean(fprs)
    auroc_list_SCOOD.append(auroc); aupr_list_SCOOD.append(aupr); fpr_list_SCOOD.append(fpr)
...
print('\n\nMean Test (SCOOD) Results!!!!!')
logger.info('\n\nMean Test (SCOOD) Results!!!!!')

print_measures_log(logger, np.mean(auroc_list_SCOOD), np.mean(aupr_list_SCOOD), np.mean(fpr_list_SCOOD), method_name=args.method_name)

normalizer=trn.Normalize(mean, std)

# /////////////// Textures ///////////////

ood_loader =get_dataloader('dtd',normalizer,args.test_bs)
print('\n\nTexture Detection')
logger.info('\n\nTexture Detection')
get_and_print_results(logger,ood_loader)

# /////////////// SVHN /////////////// # cropped and no sampling of the test set
ood_loader =get_dataloader('svhn',normalizer,args.test_bs)
print('\n\nSVHN Detection')
logger.info('\n\nSVHN Detection')
get_and_print_results(logger,ood_loader)

# /////////////// CIFAR /////////////// # cropped and no sampling of the test set
...
ood_loader =get_dataloader(dout,normalizer,args.test_bs)
print('\n\nCIFAR Detection')
logger.info('\n\nCIFAR Detection')
get_and_print_results(logger,ood_loader)

# /////////////// Places365 ///////////////
ood_loader =get_dataloader('place365',normalizer,args.test_bs)
print('\n\nPlaces365 Detection')
logger.info('\n\nPlaces365 Detection')
get_and_print_results(logger,ood_loader)

# /////////////// LSUN-C ///////////////
ood_loader =get_dataloader('lsun',normalizer,args.test_bs)
print('\n\nLSUN_C Detection')
logger.info('\n\nLSUN_C Detection')
get_and_print_results(logger,ood_loader)

# /////////////// LSUN-R ///////////////
ood_loader =get_dataloader('lsunR',normalizer,args.test_bs)
print('\n\nLSUN_Resize Detection')
logger.info('\n\nLSUN_Resize Detection')
get_and_print_results(logger,ood_loader)

# /////////////// iSUN ///////////////
ood_loader =get_dataloader('isun',normalizer,args.test_bs)
print('\n\niSUN Detection')
logger.info('\n\niSUN Detection')
get_and_print_results(logger,ood_loader)

# /////////////// MNIST ///////////////
ood_loader =get_dataloader('mnist',normalizer,args.test_bs)
print('\n\nMNIST Detection')
logger.info('\n\nMNIST Detection')
get_and_print_results(logger,ood_loader)

# /////////////// KMNIST ///////////////
ood_loader =get_dataloader('kmnist',normalizer,args.test_bs)
print('\n\nKMNIST Detection')
logger.info('\n\nKMNIST Detection')
get_and_print_results(logger,ood_loader)

# /////////////// FASHIONMNIST ///////////////
ood_loader =get_dataloader('fasionmnist',normalizer,args.test_bs)
print('\n\nFASHION MNIST Detection')
logger.info('\n\nFASHION MNIST Detection')
get_and_print_results(logger,ood_loader)

# /////////////// STL ///////////////
ood_loader =get_dataloader('stl10',normalizer,args.test_bs)
print('\n\nSTL Detection')
logger.info('\n\nSTL Detection')
get_and_print_results(logger,ood_loader)
...
print_measures_log(logger, np.mean(auroc_list), np.mean(aupr_list), np.mean(fpr_list), method_name=args.method_name)
...
get_and_print_results_syn(logger,ood_loader)
...
print('\n\nMean Test(SYN)Results!!!!!')
logger.info('\n\nMean Test(SYN) Results!!!!!')

print_measures_log(logger, np.mean(auroc_list_syn), np.mean(aupr_list_syn), np.mean(fpr_list_syn), method_name=args.method_name)
