...
if [ "$1" = "MSP" ]; then
	...
	model=${array[1]}
		...
		m_in=-27
	            ...
	            echo "---------------"$m_in"------"$m_out"--------------------"
	            CUDA_VISIBLE_DEVICES=$gpu python OE_train_im_resc.py $data --model $model --score $score --seed $seed --m_in $m_in --m_out $m_out --trial $trial
	            CUDA_VISIBLE_DEVICES=$gpu python OE_test_SC_resc.py --method_name ${dm}_s${seed}_$1 --num_to_avg 1 --score $score --trial $trial
           done
        done
    done
