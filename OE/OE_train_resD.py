...
if args.dataset == 'cifar10':
    train_data_in= dset.CIFAR10('./data/cifar10', train=True, transform=MultiCropTransform(train_transform,n))
    test_data = dset.CIFAR10('./data/cifar10', train=False, transform=test_transform)
    num_classes = 10
    prior_gamma1=torch.tensor([0.11510667,0.03620667,0.099436667,0.20430333,0.03579667,0.11949,0.07505667,0.05346,0.09908333,0.16206]).cuda()**0.75
    prior_gamma2=torch.tensor([0.11510667,0.03620667,0.099436667,0.20430333,0.03579667,0.11949,0.07505667,0.05346,0.09908333,0.16206]).cuda()**0.75
    ...
    load_prior='balance_'+args.dataset+'.npy'
...
def train():
    ...
    for (in_set, labels), (out_set, _) in zip(train_loader_in, train_loader_out):
        in_set_all=in_set[0]
        target_all=labels
        for i in range (n-1):
        ...
        if args.score == 'energy':
            ...
            loss += 5.0 * -(x[in_set_all.size(0):].mean(1) - torch.logsumexp(x[in_set_all.size(0):], dim=1)).mean()
...
with open(os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model + '_s' + str(args.seed) +
                                  '_' + save_info+'_dstemp_'+str(args.dstemp)+ '_quantile_' + str(args.quantile) +'_loss_mode_'+str(args.loss_mode)+'_times_'+str(args.n)+ '_beta_' + str(args.beta1)+ str(args.beta2) + '_OE_trial_' + str(
...
for epoch in range(0, args.epochs):
    ...
    torch.save(net.state_dict(),
               os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model + '_s' + str(args.seed) +
                            '_' + save_info + '_epoch_' + str(epoch)+'_dstemp_'+str(args.dstemp)+ '_quantile_' + str(args.quantile)+'_loss_mode_'+str(args.loss_mode)+'_times_'+str(args.n)+ '_beta_' + str(args.beta1)+ str(args.beta2) + '_OE_trial_' + str(
                   args.trial) + '.pt'))

    # Let us not waste space and delete the previous model
    prev_path = os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model + '_s' + str(args.seed) +
                             '_' + save_info + '_epoch_' + str(epoch - 1)+'_dstemp_'+str(args.dstemp)+ '_quantile_' + str(args.quantile) +'_loss_mode_'+str(args.loss_mode)+'_times_'+str(args.n)+ '_beta_' + str(args.beta1)+ str(args.beta2)+ '_OE_trial_' + str(args.trial) + '.pt')
