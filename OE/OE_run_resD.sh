...
if [ "$1" = "MSP" ]; then
	...
	model=${array[1]}
		...
		n=8
	            ...
	            echo "---------------"$m_in"------"$m_out"--------------------"
	            CUDA_VISIBLE_DEVICES=$gpu python OE_train_resD.py $data --model $model --score $score --seed $seed --m_in $m_in --m_out $m_out  --trial $trial --beta1 $beta1 --beta2 $beta2 --loss_mode $loss_mode --dstemp $dstemp --quantile $quantile --n $n --step_size $step_size
