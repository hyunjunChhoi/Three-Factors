...
if [ "$1" = "MSP" ]; then
	...
	model=${array[1]}
		...
		n=8	
	            ...
	            echo "---------------"$m_in"------"$m_out"--------------------"
	            CUDA_VISIBLE_DEVICES=$gpu python OE_train_im_resD.py $data --model $model --score $score --seed $seed --m_in $m_in --m_out $m_out --trial $trial --beta1 $beta1 --beta2 $beta2 --loss_mode $loss_mode --dstemp $dstemp --quantile $quantile --n $n --step_size $step_size
	            CUDA_VISIBLE_DEVICES=$gpu python OE_test_SC_resD.py --method_name ${dm}_s${seed}_$1 --num_to_avg 1 --score $score --trial $trial --beta1 $beta1 --beta2 $beta2 --loss_mode $loss_mode --dstemp $dstemp --quantile $quantile --n $n
	     #done
	   done         
        done
    done
