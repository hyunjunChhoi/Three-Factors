...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, pooling='avgpool', norm=nn.BatchNorm2d, return_features=False):
        ...
        self.class_center=nn.Parameter(torch.Tensor(num_classes,128))
        self.head_fc=nn.Sequential(nn.Linear(512, 512), nn.ReLU(), nn.Linear(512, 128))
    ...
    def intermediate_forward(self, x):
        ...
        p4 = self.pooling(h4) # (512,1,1)
        p4 = p4.view(p4.size(0), -1) # (512)
        #projected_f = self.forward_projection(p4)
    ...
    def forward_all(self, x):
        ...
        logits = self.forward_classifier(p4)
        projected_f = self.forward_projection(p4)
        #print((self.linear.weight.T).size())
        center = F.normalize(self.head_fc(self.linear.weight), dim=1)
        #print(center.size())
        center1 =F.normalize(self.class_center, dim=1)
        #print(center1.size())
        return logits, projected_f, center
